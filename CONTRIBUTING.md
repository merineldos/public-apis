...
> We truly appreciate the large number of pull requests and the involvement of our amazing community. However, some pull requests are submitted to promote commercial APIs that offer paid solutions. This API list is not a marketing tool, but a tool to help the
...
* `OAuth` - _The API uses OAuth for authentication._
* `apiKey` - _The API requires a private key string/token._
* `X-Mashape-Key` - _The authentication header must include this key._
* `No` - _The API does not require authentication._
* `User-Agent` - _The client must set this header in requests._


\* Accepted values for the `CORS` field:

* `Yes` - _The API supports CORS._
* `No` - _The API does not support CORS._
* `Unknown` - _It is unknown whether CORS is supported._
...
* Avoid submitting newer versions of APIs that are already listed, the old version of the API gets deprecated.
